...
def test_dense_reward_orders_approach_contact_grasp_and_pull(monkeypatch: pytest.MonkeyPatch, legacy: bool) -> None:
    ...
    env = SimpleNamespace(num_envs=1, device="cpu", step_dt=0.1, scene=robots)
    robot_cfgs = (
...
def test_dense_reward_filters_contact_and_resets_only_selected_envs(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    env = SimpleNamespace(num_envs=num_envs, device="cpu", step_dt=1.0 / 30.0, scene=robots)
...
def test_task_config_uses_one_reward_and_matching_success_threshold() -> None:
    """Reward and termination should share the X-separation goal without auxiliary penalties."""
    cfg = ShoelaceEnvCfg()

    assert [field.name for field in dataclasses.fields(RewardsCfg)] == ["dense_task"]
