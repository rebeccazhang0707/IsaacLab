...
def test_cable_inertia_preserves_mass_and_unselected_bodies(regularization: float) -> None:
    ...
    shoelace_physics.configure_cable_inertia(builder, [cable, anchor], regularization)
...
def test_proxy_inertia_survives_newton_validation(detailed: bool, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert regularization == shoelace_physics.CABLE_INERTIA_REGULARIZATION == 1.0e-6
    builder = newton.ModelBuilder()
    ...
    shoelace_physics.configure_cable_inertia(builder, [cable], regularization)
...
def test_invalid_proxy_inertia_is_rejected(regularization: float) -> None:
    ...
    with pytest.raises(ValueError, match="finite and nonnegative"):
        shoelace_physics.configure_cable_inertia(newton.ModelBuilder(), [], regularization)
