...
from .rewards import shoelace_grasp_quality
from .utils import tail_outward_x, tail_x_separation, untying_metrics
...
def shoelace_success(
    ...
    This geometry-only helper follows ``reb/newton_shoelace_demo`` and adds an X-separation requirement.
    The default task additionally requires cooperative pulling through ``shoelace_bilateral_pull_success``.
...
class shoelace_bilateral_pull_success(ManagerTermBase):
    ...
    def __call__(
        ...
        alpha = 1.0 - math.exp(-env.step_dt / grasp_filter_time_constant)
        filtered = torch.where(
            torch.isfinite(self._filtered_grasp), self._filtered_grasp + alpha * (grasp - self._filtered_grasp), grasp
        )
        self._filtered_grasp.copy_(torch.where(finite.unsqueeze(1), filtered, self._filtered_grasp))
