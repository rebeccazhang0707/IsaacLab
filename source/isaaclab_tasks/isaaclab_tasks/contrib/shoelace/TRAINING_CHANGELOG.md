...
# Dual-Franka shoelace training changelog

This file preserves detailed experiment configurations, metrics, failures, and decisions. See
[`README.md`](README.md) for the brief task architecture and recommended training workflow.
...
Commit `1c89eb4382cd5dda79302055ca44af58d4891fa2` freezes the pre-socket, pre-DAgger 62-observation environment
that loads the original level-91 `model_20.pt`. The `shoelace_eval_model20.sh` wrapper evaluates that artifact in a
temporary worktree at the frozen commit, so it remains valid after the active branch moves to the 68-observation
interface.

The training acceptance target is one uninterrupted fresh curriculum run that promotes automatically from level 0
through level 90. The short continuation commands below are retained as experiment provenance and regression
evidence; they are not the intended end-to-end reproduction procedure. This complete fresh-run reproduction has not
yet been rerun on the frozen branch and is therefore not claimed as a completed result.

## Frozen milestone: level-91 contact acquisition and pull

Optimization was stopped on 2026-09-08. The frozen artifact is:

```text
logs/rsl_rl/shoelace_dual_franka/
  2026-09-08_10-27-25_socketobs68_gripperrate020_l92_75_l91_25_fixedstats_h240_lr1e5_4gpu64_1_seed1920/
  model_0.pt
SHA-256: 06c259cb7b3a6fffd35f9fc9bccbf085af9ddd35fd9e19cbc4aebb644b47b80b
```

This is a deliberately narrow milestone. With grasp assistance disabled and ADMM two-way coupling enabled at
evaluation time, the policy can autonomously align both pre-positioned tails with the strict hand-frame sockets,
close both grippers, retain the tails, and satisfy the exact untying success termination from reset level 91. It
does not solve level 92 or the full level-96 approach reset, so it is not an end-to-end shoe-untying policy.

### Exact level-91 reset contract

Level 91 is deterministic at reset; it contains no joint or cable randomization. Both cable halves return to the
same authored knotted pose with zero segment velocity. Both robot joint velocities are zero, and each arm's joint
position target is set to its reset position. Both fingers start at `0.010 m` per finger, or approximately
`0.020 m` total aperture per gripper. The seven arm joint positions are:

```text
left  = ( 0.3866130, -0.0028115, -0.5242480, -2.5813480,
          1.1450305,  2.5587900, -0.1579730)
right = (-0.5077370,  0.0150884,  0.5040381, -2.5529410,
         -1.2120227,  2.6084389,  1.6128466)
```

The left pose is the precomputed 50% joint interpolation from the settled grasp pose toward the first approach
pose. The right pose is the precomputed 1.25% interpolation from the final contact-learning pose toward its first
approach pose. Immediately after reset, the measured tail-to-hand vectors are approximately
`(-1.68,+2.20,+6.33) mm` on the left and `(-13.89,-1.93,-0.75) mm` on the right. This reset therefore tests
near-contact socket acquisition and pulling, not long-range approach.

### Training lineage and physics provenance

The artifact was not trained from scratch. Its retained behavior came through the following chain:

1. The original 62-observation ADMM contact policy `2026-09-07_15-26-01_..._seed1140/model_20.pt` supplied
   successful level-91 rollouts.
2. Behavior cloning and two DAgger passes combined the approach labels with 58 successful level-91 trajectories
   (`10,981` retention transitions). The accepted BC artifact was
   `stabilization_inputs/model20_fullreset_bc_dagger2_prodref10_retain20_seed1474.pt`.
3. Low-learning-rate PPO with frozen actor observation statistics consolidated the old contact distribution.
4. `shoelace_expand_socket_observation.py` expanded the actor/critic interfaces from 62/63 to 68/69 while
   preserving the old network output when the six new socket values are zero.
5. The ADMM-trained 68-observation parent was
   `2026-09-08_09-18-37_socketobs68_l92_75_l91_25_fixedstats_h240_lr1e5_4gpu64_1_seed1770/model_0.pt`.
6. The frozen artifact is one PPO update after that parent: four GPUs, 64 environments per rank, 240 rollout
   steps, one epoch, one minibatch, learning rate `1e-5`, `gamma=0.99`, `lambda=1.0`, zero entropy coefficient,
   fixed actor observation statistics, fixed arm standard deviation `0.1`, 75% level-92 resets, 25% level-91
   replay, seed 1920, and a `0.20 m/s` binary gripper-target rate limit.

The final update in step 6 used the proxy coupler. This is confirmed by both its startup banner and saved
`params/env.yaml`; the run name's omission of `proxy` must not be interpreted as ADMM training. The parent and
earlier contact policy were trained with ADMM, and every acceptance result below explicitly evaluates the frozen
artifact with `--coupling_mode admm`. The exact final-update command was:
...
uv run isaaclab train_multigpu \
  ...
  --num_gpus 4 \
  --tee 3 \
  --log_dir /tmp/shoelace_gripperrate020_seed1920_torchrun \
  --task IsaacContrib-Shoelace-DualFranka \
  --num_envs 64 \
  --max_iterations 1 \
  --checkpoint logs/rsl_rl/shoelace_dual_franka/2026-09-08_09-18-37_socketobs68_l92_75_l91_25_fixedstats_h240_lr1e5_4gpu64_1_seed1770/model_0.pt \
  --run_name socketobs68_gripperrate020_l92_75_l91_25_fixedstats_h240_lr1e5_4gpu64_1_seed1920 \
  --seed 1920 \
  --visualizer none
```

The non-default PPO and curriculum settings were temporarily checked into the source used by the run and are
captured in its `params/agent.yaml`, `params/env.yaml`, and `git/IsaacLab_newton.diff`; ordinary source defaults
were restored afterwards. The recorded Git diff does not include untracked files, so those files plus the ignored
checkpoint directory must be preserved separately until the current source is committed.

With the restored source defaults, the following standard RSL-RL command expresses the same final update entirely
through CLI overrides. A zero-iteration, one-environment configuration smoke test confirmed that these overrides
resolve to the 68/69 observation interface, fixed-statistics actor, proxy coupler, and expected parent checkpoint:

```bash
PARENT=logs/rsl_rl/shoelace_dual_franka/2026-09-08_09-18-37_socketobs68_l92_75_l91_25_fixedstats_h240_lr1e5_4gpu64_1_seed1770/model_0.pt
uv run isaaclab train_multigpu \
  --rl_library rsl_rl \
  --num_gpus 4 \
  --tee 3 \
  --log_dir logs/torchrun/milestone_rate020_reproduction_seed1920 \
  --task IsaacContrib-Shoelace-DualFranka \
  --num_envs 64 \
  --max_iterations 1 \
  --checkpoint "$PARENT" \
  --run_name milestone_rate020_reproduction_seed1920 \
  --seed 1920 \
  --visualizer none \
  agent.num_steps_per_env=240 \
  agent.save_interval=1 \
  agent.actor.class_name=isaaclab_tasks.contrib.shoelace.agents.models:FixedObservationStatisticsMLPModel \
  agent.actor.distribution_cfg.init_std=0.1 \
  agent.actor.distribution_cfg.arm_action_scale=0.7 \
  agent.actor.distribution_cfg.gripper_logit_scale=2.0 \
  agent.actor.distribution_cfg.learn_std=false \
  ...
  agent.algorithm.lam=1.0 \
  agent.algorithm.entropy_coef=0.0 \
  agent.algorithm.normalize_advantage_per_mini_batch=false \
  env.coupling_mode=proxy \
  env.grasp_assist_enabled=false \
  env.actions.left_gripper.maximum_velocity=0.2 \
  env.actions.right_gripper.maximum_velocity=0.2 \
  env.curriculum.pull_to_grasp.params.initial_level=92 \
  env.curriculum.pull_to_grasp.params.minimum_episodes=1000000 \
  env.curriculum.pull_to_grasp.params.current_level_fraction=0.75 \
  'env.curriculum.pull_to_grasp.params.current_level_fraction_schedule=[0.75]' \
  'env.curriculum.pull_to_grasp.params.replay_level_weights=[1.0]'
```

This reproduces the experiment contract, not the checkpoint byte sequence; distributed GPU contact simulation and
PPO are not bitwise deterministic. Replacing `env.coupling_mode=proxy` with `env.coupling_mode=admm` defines a new
production-physics continuation and is not a reproduction of the frozen artifact's final update.

### Fixed evaluation protocol and result

The policy was evaluated deterministically for at most 420 policy steps in 64 parallel environments, without an
approach prefix or grasp assistance. The reusable command is:

```bash
CHECKPOINT=logs/rsl_rl/shoelace_dual_franka/2026-09-08_10-27-25_socketobs68_gripperrate020_l92_75_l91_25_fixedstats_h240_lr1e5_4gpu64_1_seed1920/model_0.pt
uv run isaaclab -p scripts/demos/shoelace_open_loop_pull.py \
  --visualizer none \
  --coupling_mode admm \
  --num_envs 64 \
  --approach_steps 0 \
  ...
  --initial_level 91 \
  --seed 2010 \
  --run_label milestone_rate020_l91_seed2010 \
  2>&1 | rg '^SHOELACE_HYBRID_RESULT='
```

The corresponding standard RSL-RL player command is below. `--train_env_cfg` is required because the task's normal
play-mode override selects the complete level-96 reset. This player runs until interrupted and is intended for
visual or qualitative inspection; use the command above for finite, aggregated task counts.
...
uv run isaaclab play \
  ...
  env.actions.left_gripper.maximum_velocity=0.2 \
  env.actions.right_gripper.maximum_velocity=0.2 \
  env.curriculum.pull_to_grasp.params.initial_level=91 \
  env.curriculum.pull_to_grasp.params.minimum_episodes=1000000 \
...
Five independent launches, including a repeated seed-2010 launch, gave:

| Reset | Seed/run | Strict socket acquisition | Exact success | Lost grasp | Safe |
|---:|---|---:|---:|---:|---:|
| 91 | 2010-A | 64/64 | 40/64 | 24/64 | yes |
| 91 | 2011 | 64/64 | 43/64 | 21/64 | yes |
| 91 | 2012 | 64/64 | 39/64 | 25/64 | yes |
| 91 | 2013 | 64/64 | 39/64 | 25/64 | yes |
| 91 | 2010-B | 64/64 | 46/64 | 18/64 | yes |
| **L91 total** | **five launches** | **320/320** | **207/320 (64.7%)** | **113/320** | **yes** |
| 90 | 2010 | 64/64 | 50/64 | 14/64 | yes |
| 80 | 2010 | 64/64 | 22/64 | 42/64 | yes |
| 92 | 2010 | 0/64 | 0/64 | 64/64 | yes |

The repeated seed did not reproduce bit for bit (`40/64` versus `46/64`), which is expected from the current
parallel GPU contact solve. The milestone is statistically reproducible: all five L91 launches acquired both
strict sockets in every environment and exact success stayed between `60.9%` and `71.9%`. Level 90 and level 80
are supporting one-batch checks, not multi-seed claims. At level 92, all environments reached the looser Euclidean
grasp condition, but none entered the strict right socket; the best right socket error was `15.366 mm` against the
`12 mm` acquisition threshold. This is the explicit upper boundary of the milestone.

Checkpoint evaluation is reproducible from the current worktree and artifact. A clean, from-scratch reproduction
of the complete training lineage has not been run and is not claimed. Cross-machine reproduction additionally
requires transferring the ignored checkpoint and committing or otherwise archiving the current untracked source
files.
...
### Actor observation: `policy`, 68 dimensions
...
| 7 | `grasp_socket_error` | 6 | 50 | Signed tail error from the calibrated contact socket, one XYZ vector per hand frame [m]. |
| 8 | `tails_to_knot` | 6 | 10 | Tail positions relative to the knot center, two XYZ vectors in environment axes [m]. |
| 9 | `tail_velocities` | 6 | 1 | Linear velocities of the two free-tail regions in environment axes [m/s]. |
| 10 | `last_action` | 14 | 1 | Previous executed EMA arm commands and raw binary-gripper actions. |

Dimension check:

```text
4 * 7 arm states + 2 * 1 finger states + 4 * 6 lace states + 14 previous actions = 68
```

The actor does **not** directly observe contact forces, a Boolean grasp flag, throat density, episode
phase, or the reference pull directions. It must infer grasping from tail/TCP and signed contact-socket geometry,
finger position, lace motion, and action history.

### Critic observation: 69 dimensions

The asymmetric critic receives the complete 68-dimensional `policy` group plus:
...
passive joint for the Newton articulation. The selected binary target is rate-limited around the measured driven
finger position at `0.20 m/s`, preventing a full target jump from becoming a one-step contact impulse.
...
```text
hand-frame socket error_i <= 0.012 m
0.0003 m <= q_i <= 0.0025 m
```

The `0.0015 m` close target lies inside this strict interval. The socket condition is used only to acquire and latch
the grasp. Retention then uses the wider `0.020 m` Euclidean tail-to-TCP condition so normal cable motion under load
does not invalidate a sound grasp. Success permits finger rebound up to `0.0035 m`, and only while the tail remains
within the tighter `0.015 m` success distance and every untying margin is complete. This accommodates the
`0.003 m` shoelace contact thickness under load without weakening training-time grasp shaping. The lower bound
rejects over-closed states below `0.0003 m`; proximity is always required, so aperture alone is not a grasp.
...
The first distributed launch exited during native Newton scene parsing before it created an environment. The
elastic launcher's single configured restart then initialized all eight ranks normally; the failed attempt produced
no rollout or checkpoint data. The completed attempt collected 4,915,200 transitions over sixty updates. With the
actor frozen, it promoted levels 86, 87, and 88 and ended at level 89 with 20% frontier exposure. Level-87 completed
windows were approximately 60--70%; level 88 passed all three exposure stages with only a 1--2-point margin over
the 50% production threshold. The final mixed batch contained about 60.0% success, 36.6% lost grasp, 3.9%
insufficient separation, and no unsafe termination. Every actor tensor remained bitwise identical to the input;
only the six critic MLP tensors changed, all remained finite, and the optimizer retained the requested `3e-4` rate.

The saved trace datasets lacked the critic's single throat-density input, so that value was reconstructed from the
stored unclipped throat-progress states and the authored 156-free-segment topology. On 194,841 held-out states from
nine level-80-to-86 datasets, this reconstruction reproduced the originally stored critic values with mean absolute
error about `3e-6`. The warm-up increased explained variance from 0.602 for the input critic to 0.671 for
`model_59.pt`, although its earlier-level RMSE reached a minimum around `model_25.pt` and then developed a negative
bias. A same-trajectory frontier gate therefore compared both critics rather than selecting the final checkpoint by
iteration number. Four level-90 seeds produced 258/512 policy successes; `model_59.pt` improved weighted critic
explained variance from 0.608 to 0.648 and RMSE from 13.53 to 13.09. Four level-91 seeds produced 202/512 successes;
it improved explained variance from 0.600 to 0.630 and RMSE from 13.73 to 12.75. The later critic was consequently
accepted despite its less favorable error bias on earlier levels.

The ordinary frozen-backbone actor still exposes both binary-gripper rows in its trainable output layer. Earlier
experiments showed that moving either row across zero can destroy acquisition, even when the arm update is useful.
`FrozenGripperOutputHeadFixedObservationStatisticsMLPModel` therefore preserves the existing checkpoint layout,
freezes the observation statistics and hidden backbone, and masks gradients for output rows 6 and 13. It requires a
fresh optimizer so restored moments cannot move those rows. A two-update, eight-GPU contract gate used 240-step
rollouts, fixed arm standard deviation `0.10`, one full-batch PPO epoch, and `1e-5` learning rate from level 90.
After its first update, only the final-layer weight and bias had changed; the twelve arm rows moved by at most
`1.0001e-5`, while both gripper rows, the backbone, normalizer, and fixed standard deviation remained bitwise
unchanged. A paired four-seed fixed-level gate measured 260/512 successes for the input and 256/512 for the updated
actor, rejecting a material first-update regression.

The contract run also demonstrated a distributed logging trap. Its reported `last_window_success_rate` was 0.1699
while some ranks had not yet completed their first 128-episode window, because the logger averaged valid per-rank
rates with the untouched zero initialization from other ranks. The fixed-level paired gate above contradicts a true
17% success rate, so the averaged startup metric is not used for policy selection. A fresh 300-update continuation
from the pre-gate input was launched under
`logs/rsl_rl/shoelace_dual_franka/2026-09-07_15-26-01_curriculum97_armoutputhead_h240_lr1e5_start90_thresh50_admm_8gpu128_300_seed1140`.
It uses eight GPUs with 128 environments per rank, 240-step rollouts, the accepted `model_59.pt` critic, fresh Adam,
and the arm-only output-head contract. Fixed-level multi-seed gates, not partially initialized distributed window
averages, remain the acceptance criterion for its checkpoints.

That continuation promoted level 90 and reached level 91 at 20% frontier exposure. Seven subsequent distributed
window reports ranged from 31.01% to 41.41% and showed no clear approach to the 50% exposure gate, so the run was
stopped after the complete `model_20.pt` checkpoint rather than spending the remaining allocation on the mixed
online signal. At that point the actor backbone, observation statistics, standard deviation, and gripper output
rows were still bitwise identical to the input. Only the twelve arm rows in the final layer had moved, by at most
`1.87e-4`, and all actor tensors remained finite. On saved held-out observations, the learned right local-`x/z`
shift remained outward and upward, agreeing with the earlier causal intervention rather than erasing it.

A new four-seed, fixed-level-91 gate then showed that the noisy curriculum windows had hidden real policy progress.
On seeds 1150--1153, the input produced 192/512 successes (37.50%), 299 lost grasps, and 21 insufficient-separation
terminations. `model_16.pt` produced 219/512 (42.77%), 270 lost grasps, and 23 insufficient-separation terminations;
`model_20.pt` produced 234/512 (45.70%), 257 lost grasps, and 21 insufficient-separation terminations. Every episode
completed, bilateral acquisition remained complete, and neither missed-grasp nor unsafe termination occurred.
The monotonic prospective improvement accepts `model_20.pt` as the current continuation point, but its 4.3-point
shortfall from the production threshold does not yet justify claiming level-91 mastery.

A short continuation from `model_20.pt` then tested whether the trend persisted under a new training seed. It
replayed level 90, re-entered level 91 at 20% exposure, and was stopped at `model_30.pt` for another fixed gate.
On the same held-out level-91 seeds, success declined monotonically from 234/512 (45.70%) at model 20 to 223/512
(43.55%) at model 27 and 211/512 (41.21%) at model 30. Diagnostic actors copied either only model 30's right
local-`x/z` rows or all six right-arm rows onto model 20; they produced 201/512 (39.26%) and 199/512 (38.87%)
successes respectively. The additional outward/up update itself had therefore crossed the useful range, rather
than degradation being caused only by unrelated arm rows.

Eight new `model_20.pt` level-91 datasets contained 414 successes, 557 lost grasps, and 53 other terminations.
Bilateral acquisition occurred at control step three or four in every environment. At reset step zero, the
successful trajectories' sampled residual differed from lost-grasp trajectories by `-0.0240` in right local `x`
and `+0.0223` in right local `z`; both signs agreed on all eight seeds. The signal weakened after acquisition and
was not consistently useful across the later trajectory. This explains how PPO could initially help and then
regress: delayed terminal credit spread a contact-entry correction over states where continuing the same motion
was harmful.

One-step causal interventions from `model_20.pt` confirmed the timing hypothesis. A common-seed dose screen
converted lost grasps into successes, and selected an additional right local `(x, z)=(-0.125,+0.075)` action only
at reset step zero. On four entirely new paired seeds, the unmodified policy produced 207/512 successes (40.43%),
273 lost grasps, and 32 insufficient-separation terminations. The intervention produced 261/512 successes (50.98%),
211 lost grasps, and 40 insufficient-separation terminations; per-seed success gains were 10, 21, 15, and 8. Every
episode completed with bilateral acquisition and no missed-grasp or unsafe termination. The intervention narrowly
passes the production threshold, but its 0.98-point margin requires policy distillation and retention gates before
level 91 can be accepted as mastered.

An eight-seed paired gate refined the selected first-step correction to right local
`(x, z)=(-0.125,+0.100)`. On seeds 1220--1227 with the same seed-to-GPU mapping, `model_20.pt` produced 399/1,024
successes (38.96%), 557 lost grasps, and 68 insufficient-separation terminations. The corrected policy produced
525/1,024 successes (51.27%), 410 lost grasps, and 89 insufficient-separation terminations. Seven of eight seeds
improved, with per-seed success differences of `+19,+18,+15,+23,-2,+27,+11,+15`; neither condition had an unsafe,
missed-grasp, or unfinished episode. This is a causal contact-entry gain, but its 1.27-point production-threshold
margin remains too small to treat as robust mastery.

Cross-level runtime gates then bounded where that correction transfers. On one common seed, levels 80, 83, 86,
and 90 changed from 68, 105, 104, and 60 successes out of 128 to 58, 107, 104, and 84 respectively. The correction
is therefore useful or neutral over the contact-bridge region but harmful at level 80. An initial frozen-output-head
fit had incorrectly assigned zero-delta retention targets to levels 83--86, directly conflicting with the positive
level-91 target. Relabeling levels 83--86 as positive removed that conflict, but the linear head still leaked an
executed reset-step delta of approximately `(-0.095,+0.076)` into held-out level-80 observations. Both linear
candidates were rejected before simulation.

A more expressive offline fit used the existing split-backbone actor. Its gripper branch remained bitwise equal to
`model_20.pt`; only the arm MLP was distilled. Held-out targets requested the correction at reset step zero for
levels 83--91, zero correction at level 80, and zero correction after step zero at every sampled level. The selected
fit reproduced the level-91 correction with component-error p95 `4.8e-4`, limited level-80 reset leakage to
`8.0e-5`, and limited later target-row and other-arm-row drift to p95 `0.00218` and `0.00097`. Its checkpoint is
`stabilization_inputs/curriculum97_model20_l83_91_xz_split_distilled_xm0125_zp0100_step0.pt`.

Policy-only simulation confirmed the frontier effect but not yet the full retention gate. Across the same eight
level-91 seeds, the split candidate produced 539/1,024 successes (52.64%), 354 lost grasps, and 131
insufficient-separation terminations, versus the 399/1,024 source result and 525/1,024 runtime-teacher result.
Per-seed candidate successes were 69, 74, 79, 72, 56, 66, 64, and 59. On the earlier common-seed cross-level gate,
levels 80, 83, and 86 changed from 68, 105, and 104 to 62, 101, and 98, while level 90 improved from 60 to 94.
The candidate has therefore learned a transferable contact-bridge correction rather than a complete untying
trajectory, but its 52.64% frontier rate is below the 55% safety margin and the small earlier-level regressions
require additional paired retention gates before PPO resumes.

The next phase boundary was evaluated directly rather than inferred from the level number. Level 92 changes the
left reset from 50% to 100% of its first approach interval and the right reset from 1.25% to 100% of its remaining
interval. Across eight paired seeds, `model_20.pt` produced 0/1,024 successes: 727 lost grasps, 247 insufficient-
separation terminations, and 50 missed grasps. It nevertheless acquired both tails within the deadline in 95.12%
of episodes and acquired them at some point in every episode; the mean first acquisition step was approximately
one. Its level-64 component summary showed left-tail progress `0.0819`, right-tail progress `0.0`, and separation
progress `0.0064`. Thus the failure is not simply an unreachable pregrasp reset: the policy rapidly enters contact
but cannot transfer its retained right-tail pull behavior to the new arm geometry.

The split candidate also produced 0/1,024 level-92 successes, with 872 lost grasps and 152 missed grasps. Its
deadline acquisition rate fell to 85.16%, confirming that the level-83-to-91 first-step correction extrapolates
harmfully beyond its fitted contact-bridge region. This result rejects immediate PPO continuation from the split
candidate and identifies level 92 as a phase-boundary distribution shift, not an independent physics or threshold
bug. A replacement candidate must retain the source behavior at both level 80 and level 92 while improving the
level-83-to-91 contact bridge; level-92 progress then needs its own state-conditioned approach/contact objective or
additional reset states between the current 1.25% and 100% right-arm endpoints.

Eight additional level-92 trace datasets separated 690 lost-grasp, 282 insufficient-separation, and 52
missed-grasp episodes. The relatively better insufficient-separation trajectories used a higher right local-`x`
residual than lost-grasp trajectories: `+0.00956` over steps 0--3 and `+0.01009` over steps 0--7, with the positive
sign agreeing on all eight seeds. The difference remained `+0.00503` over post-acquisition steps 16--63, again on
all eight seeds. This direction is opposite the one-step negative-`x` correction that helps level 91. The phase
boundary therefore requires a state-conditioned change from contact-entry stabilization to sustained right-tail
pulling; one correction cannot safely be broadcast across both geometries. A prospective level-92 action scan is
required before changing rewards, adding more reset levels, or launching another long PPO run.

### Hybrid imitation pivot and expert-controller diagnosis

Reset geometry was measured in each hand frame before committing to behavior cloning. The level-91 right
tail-to-TCP vector was approximately `(-13.9, -1.9, -0.8) mm`, level 92 jumped to
`(+6.3, +4.3, +3.4) mm`, and level 93 changed to `(+1.3, -2.0, +18.8) mm`. Levels 94--96 then moved
monotonically farther along the approach direction. Thus level 92 is not merely a farther version of level 91:
the TCP starts on the other side of the tail, and a direct Cartesian correction crosses the cable.

The first no-assist ADMM scripted candidate moved from level 92 directly toward the mean level-91 acquisition
socket and then pulled for 200 steps. It acquired only the left tail. The TCPs moved approximately `44.5 mm`,
while the corresponding tail projections were `-3.9/-4.1 mm`; both grasps were lost and no success gate fired.
Direct closure at the level-92 reset and pull speeds from `0.005` to `0.040 m/s` also failed. The right tail moved
opposite its commanded direction, excluding pull speed alone as the root cause.

A two-stage expert then moved to a hand-frame pregrasp waypoint before descending. A `0.030 m` local-Z clearance
at `0.010 m/s` produced the best initial socket: both tails acquired and stayed inside the retention distance for
a two-second `0.005 m/s` pull. A longer pull scan compared open loop with tail-to-TCP feedback gains of `0.02`,
`0.05`, and `0.10`. Gain `0.02` produced the best task progress: left/right tail-to-knot distances changed from
`66.6/62.3 mm` to `79.1/67.4 mm` and separation changed from `110.5 mm` to `123.4 mm`. The right grasp still
released before success. Larger gains suppressed useful motion, while open loop lost both tails.

The same translation-only controller stopped approximately `15--19 mm` from the tails at levels 93 and 96. This
identified a second limitation: the distant reset poses require the wrist orientation to converge with position;
holding each reset orientation turns the Cartesian translation into an IK/workspace limitation. The expert demo
now supports taking its approach orientation from a reference curriculum level so this hypothesis can be tested
without altering the production environment.

The shortest training route is therefore a hybrid rather than a fresh policy. Preserve `model_20.pt` as the
contact-rich initialization, collect new closed-loop approach and recovery demonstrations, mix them with successful
level-80-to-91 policy rollouts, run behavior cloning with old-policy preservation, then use a short DAgger and PPO
fine-tune. The 97 reset states remain a regression suite, but only a small set of pull, contact, near-approach,
mid-approach, and full-approach anchors need participate in the new training sampler. Four GPUs are reserved for
expert search, two for held-out validation, and two for BC work; eight-GPU PPO begins only after fixed-policy gates
pass.

### Approach convergence and right-socket isolation

Further instrumentation corrected the interim interpretation above that a `15--19 mm` residual necessarily
indicated an orientation or workspace limitation. With only 120 controller steps, the pregrasp and grasp stages
moved the TCPs by roughly `10--12 mm` and `15--17 mm`, while the tails moved by at most `3 mm`; the frozen-target
errors remained as large as `25--35 mm`. The controller had simply stopped before converging. Reusing the level-91
orientation did not improve this result. Keeping the level-96 wrist orientation, using `0.020 m/s`, and allowing up
to 360 steps per stage reached a physically executable no-assist socket with left/right distances of
`6.23/6.21 mm`. Both sides passed the grasp check after closure. This became the expert-generator approach path:
level 96, then waypoints 95 and 94, then a frozen-tail pregrasp and descent.

Two collision hypotheses were rejected. Filtering robot-to-shoe contacts did not change the residual, and filtering
only the palm capsule against the cables while preserving finger-pad contact made it worse. Neither diagnostic
filter was retained. A tail-direction indexing error in the optional hand-roll alignment diagnostic was also fixed;
the production pull directions and all results using explicitly configured reference directions were unaffected.

The first full scripted-policy handoff followed waypoints `96 -> 95 -> 94 -> 93 -> 91`, acquired both tails on the
second policy step, and moved the left tail to `168 mm` from the knot. The right tail peaked near `64 mm`, so no
success occurred before both grasps were eventually lost. Direct level-92 residual-action scans similarly converted
many lost-grasp episodes into insufficient-separation episodes but produced essentially zero right-tail progress.
Together these runs showed that approach acquisition was no longer the only missing behavior: the right-hand socket
did not transmit sustained pull load.

At the nominal expert socket, the right tail settled near hand-frame local `x=+11 mm`, outside or at the end of the
approximately `+/-8.75 mm` finger-pad half-length. A coarse negative-x target scan crossed an IK discontinuity below
`-10 mm`; a fine scan found that a `-8 mm` target settled near `x=+7.7 mm` while remaining inside the `20 mm`
retention envelope. Four 12-second pulls then isolated the remaining behavior:

| Pull speed | Feedback gain | Left/right tail pull | Left/right follow ratio | Final grasp distance | Result |
| --- | --- | --- | --- | --- | --- |
| `0.005 m/s` | `0.00` | `15.42/4.84 mm` | `0.905/0.244` | `8.83/28.01 mm` | right released |
| `0.005 m/s` | `0.02` | `16.41/5.92 mm` | `0.972/0.468` | `8.54/19.75 mm` | retained, but outside the `15 mm` success grasp gate |
| `0.010 m/s` | `0.00` | `1.75/1.77 mm` | `0.046/0.044` | `45.13/53.52 mm` | both released |
| `0.010 m/s` | `0.02` | `-3.80/5.09 mm` | `-0.242/0.256` | `25.69/27.11 mm` | both released |

The useful change in right follow ratio from approximately zero to `0.468` validates the lateral socket diagnosis,
but it is not yet a successful expert trajectory. The next search therefore changes only the right socket's local-Z
target to place the cable deeper between the pads. Success thresholds remain unchanged.

A local-Z target scan found a narrow feasible basin at `-8 mm`. That target produced a closed right-hand socket near
`(x, y, z)=(9.45, 0.54, -3.31) mm`, bilateral acquisition, and a post-hold distance of `14.89 mm`. Targets at
`-12`, `-16`, and `-20 mm` did not move monotonically deeper; they caused lateral IK deflection or lost acquisition.
A second scan reduced the commanded per-finger close position from `1.5 mm` to `1.25`, `1.0`, `0.75`, and `0.5 mm`.
None improved right-tail transmission, while the two tightest settings also degraded left-tail motion. The nominal
`1.5 mm` close target was retained. This excludes insufficient grip-force command as the current cause and avoids
using excessive closure to hide a socket-placement error.

Extending the constant-direction pull to 720 steps also failed, so duration alone was rejected. The best lateral
target in that batch was `x=-9 mm`: the left/right tail projections reached `25.30/5.03 mm`, the follow ratios were
`0.821/0.435`, and the right grasp ended at `25.85 mm`. The controller had reached a loaded kinematic equilibrium,
not a trajectory that would continue opening the knot with more identical actions.

The same `x=-9 mm, z=-8 mm` approach was handed to `model_20.pt` instead of the constant pull. Bilateral acquisition
occurred on policy step 2. The learned contact policy moved the left tail beyond its success distance to `91.38 mm`,
reduced the throat count to 74, and reached `148.08 mm` separation, but the right grasp was confirmed lost at step
88. This is useful positive transfer from the level-90/91 checkpoint, but the reached right socket
`(2.87, 2.47, 1.40) mm` was still on a different side of the cable from successful native level-91 acquisition.
The next experiment therefore removes the fitted socket and follows only collision-free curriculum-pose waypoints
to the exact level-91 hand pose before policy handoff.

The waypoint comparison confirmed that level 92 should not lie on the expert path. The route
`96 -> 95 -> 94 -> 93 -> 91` acquired bilaterally on policy step 2 and retained until step 145. It moved the left
tail to `130.20 mm` and reached `190.04 mm` separation, both beyond their success thresholds. The right tail peaked
at `72.10 mm`, the throat count reached 68, and the right grasp then released. Including level 92 prevented bilateral
acquisition, while omitting both 92 and 93 reduced retention to 94 steps. The four waypoint levels are therefore not
equivalent micro-steps: level 93 is a useful collision-free bridge, whereas level 92 is a harmful side-changing pose.
The retained route now receives only a small right-hand translation before policy handoff; the left hand is held at
the proven level-91 pose.

### Evaluation parity and curriculum interpretation

A parallel level-91 rollout initially appeared to contradict the accepted fixed-policy gates. With 128 environments,
seed 1220, stochastic arm standard deviation `0.10`, and `model_20.pt`, the expert script produced 0/128 successes
and lost every grasp. The script was still overriding the task's physics discretization from five to eight substeps.
Repeating the same rollout with the training value of five produced 65/128 exact successes, 63 lost grasps, bilateral
acquisition in every environment, and no unsafe result. The earlier independent evaluator produced 52/128 successes
on the same seed. The remaining difference is within the stochastic outcome variation caused by the two rollout
harnesses, while the zero-success result was a different physical contract. The expert script now preserves the task
substep setting by default and reports the runtime value; an explicit override remains available for solver studies.
The corrected collector then saved 58 successful trajectories from a repeated 128-environment rollout to
`logs/demonstrations/shoelace/model20_level91_seed1220.pt`. The dataset contains 10,981 aligned, finite
observation-action transitions, with trajectory lengths from 173 to 204 control steps and metadata recording five
physics substeps.

This result also bounds what can be claimed about the 97-level curriculum. The dense finger-aperture levels were
historically useful for locating and crossing several micrometre-sensitive contact-release boundaries, and the later
contact-pose levels produced the policy represented by `model_20.pt`. They are therefore valid provenance and a useful
regression suite. They are not all independent skills or mandatory future training gates: many consecutive levels
change only the reset finger aperture, several repeat the same arm pose, and level 92 is a measured geometric
discontinuity that is harmful as an expert waypoint. The shortest forward path retains the checkpoint and representative
anchor distributions, rather than replaying all 97 promotions or claiming that every historical micro-level was an
intrinsically necessary curriculum stage.

### Full-reset acquisition geometry under matched substeps

Repeating the full-reset expert diagnosis with the task's five physics substeps moved the failure boundary back to
contact entry. The raw `96 -> 95 -> 94 -> 93 -> 91` route, with each arm latched when it entered a `10 mm` final
waypoint window, produced bilateral acquisition in 43/128 environments but no success. At first acquisition, the
lost-grasp subset had mean left/right hand-frame tail vectors of approximately
`(+0.63,+0.86,+8.39)/(+5.10,-1.17,+9.90) mm`. A native level-91 rollout acquired in 128/128 environments and
produced 65 successes; its successful acquisition means were
`(-2.35,-0.17,+6.98)/(-5.45,-1.44,-4.53) mm`. Native successes and native lost grasps began from almost identical
sockets, so their outcomes diverged after acquisition. The full-reset route instead acquired the right tail on the
opposite local-X and local-Z side before the retained policy began pulling.

Rolling the level-91 reference hands to align their local frames with the measured tail tangents did not correct the
entry path. The required diagnostic rolls were approximately `-36.85/-10.70 degrees`. Four 16-environment variants,
with no early close and with `8`, `10`, or `12 mm` close windows, all produced zero bilateral acquisitions. The left
pregrasp frozen-target error was already about `27 mm`, and the final tail distances were commonly `38--50 mm`.
The failure therefore preceded the learned pull and was not fixed by a single wrist-roll transform.

A matched-seed aperture scan then changed only the commanded open position on the raw waypoint route:

| Open position per finger | Bilateral acquisition | Success | Confirmed lost grasp | Other at 300 steps |
| --- | ---: | ---: | ---: | ---: |
| `10 mm` | 15/32 | 0/32 | 6/32 | 9/32 |
| `15 mm` | 32/32 | 0/32 | 30/32 | 2/32 |
| `20 mm` | 32/32 | 0/32 | 29/32 | 3/32 |
| `25 mm` | 32/32 | 0/32 | 32/32 | 0/32 |

The wider aperture removed missed acquisitions but did not create a load-bearing grasp. With `15--25 mm` open
positions, the right acquisition mean remained near local `x=+7 mm, z=+8 mm`, rather than the native successful
`x=-5.45 mm, z=-4.53 mm` socket. This separated two mechanisms: finger clearance controls whether the cable enters
the acquisition envelope, while the side and depth of the closed socket control whether the grasp transmits pull
load. The next expert search retained the wider opening but explicitly descended from a collision-free pregrasp to
the measured native level-91 acquisition socket.

The first targeted-socket batch used the native successful acquisition means, a frozen-tail `30 mm` pregrasp, and
the level-94 hand orientation. Open positions of `15`, `20`, and `25 mm` per finger all reached the pregrasp with
only `0.5--0.8 mm` frozen-target error and little right-tail motion. During the final translation, however, every
variant left about `22.6 mm` of right TCP target error and produced zero acquisitions. Adding a three-step stationary
close to the `20 mm` variant produced only one acquisition out of 16, followed by a confirmed lost grasp. The final
right-hand translation target is therefore not reachable on that fixed wrist-orientation/IK branch; aperture alone
cannot correct it. The following batch instead tracked the complete known-reachable level-91 TCP pose, including
orientation, while retaining the wider aperture.

Tracking the complete level-91 pose in a single final waypoint also failed. With `15`, `20`, and `25 mm` open
positions, the right tail ended approximately `29 mm` from the TCP and no variant exceeded one acquisition out of
16. The tail settled near right hand-frame `(x,z)=(+18,-22) mm`, showing that the combined final translation and
orientation change swept the hand across the cable before closure. A `5 mm` early-handoff window did not help. The
expert controller was therefore split into three independently observable phases: converge to the level-91 wrist
orientation while holding a distant TCP pose, translate to a frozen-tail pregrasp, and descend at fixed orientation.
This decomposition tests a collision-safe reachable path rather than adding another curriculum interpolation.

The separated orientation stage itself was successful. All four variants converged to the level-91 hand
orientations in 111 steps with final errors below `0.09 degrees`; the largest TCP drift was `0.56 mm`, and the right
tail moved by less than `0.5 mm`. The frozen pregrasp targets also converged below `1 mm`. The subsequent target
socket descent still left `31--36 mm` of right target error and produced at most two acquisitions out of 16. This
confirmed that the failure was the statistically inferred tail-relative target, not the new orientation stage.

The next target was computed from the actual FK poses at the level-91 and level-96 resets. Relative to level 96, the
level-91 TCP translations were approximately `(+25.08,+25.97,-66.57) mm` for the left arm and
`(-41.54,+26.22,-61.29) mm` for the right arm. Those absolute, known-reachable targets replaced the acquisition-mean
socket while the collision-safe orientation stage and wider-aperture scan were retained.

Directly translating from the oriented level-94 pose to those absolute targets still produced zero acquisitions at
`15`, `20`, `25`, and `35 mm` open positions. The right TCP moved toward the target, but the right tail was swept by
approximately `15--24 mm` in the same world-Y direction and ended `20--26 mm` from the TCP. Increasing the finger
opening beyond the earlier acquisition threshold did not reduce this motion, so the collision was caused by the
oblique hand path rather than insufficient space between the pads. The absolute target remained valid, but the
approach direction did not. A tool-axis pregrasp was added at `target_tcp - local_z * clearance`, followed by a
straight local-Z descent to the same level-91 target.

The local-Z waypoint did not resolve the kinematic branch. Its pregrasp target required a lateral combination that
the right arm did not reach at the fixed level-91 orientation; the following descent again swept the right tail by
roughly `23--29 mm`, and all four open-position variants produced zero acquisitions. The next decomposition used a
world-Z waypoint instead: first match the level-91 world-X/Y coordinates at approximately the existing level-94
height, then descend vertically. Clearances of `25`, `30`, and `35 mm` bracketed the measured level-94-to-level-91
height difference, so this scan tested a reachable horizontal-then-vertical path without changing the final target.

The world-Z path also produced zero acquisitions. Its high waypoint moved the left tail by as much as
`10--19 mm`, while the right descent either swept the tail or stalled before reaching the target. These failures
showed that piecewise Cartesian geometry was repeatedly selecting poor local IK branches even though both endpoint
joint states were known and valid. Further hand-authored Cartesian offsets were stopped.

The expert generator then added diagnostic FK waypoints sampled from a linear interpolation of the authored arm
joint states. These poses are tracked through the unchanged 14-dimensional Cartesian action interface; they do not
teleport the runtime robot, change the production action contract, or add reset levels to the training curriculum.
A single-waypoint level-96-to-95 smoke test completed normally. The first contact-entry scan interpolated densely
from level 93 to level 91 after the collision-free `96 -> 95 -> 94 -> 93` route, comparing 4, 8, 16, and 32 FK
waypoints with shorter per-waypoint horizons as density increased.

All interpolation densities completed, but none improved the contact entry. The 4-, 8-, 16-, and 32-waypoint
variants produced 2, 1, 1, and 2 bilateral acquisitions out of 16, respectively, with zero successes. The right
tail remained about `20--22 mm` from the TCP at handoff. Dense kinematically feasible pose references therefore did
not supply the missing contact-aware close behavior. The next batch returned to the raw wider-aperture route that
had already produced 32/32 acquisitions, stopped pose tracking outside contact, and varied the distance at which
each gripper began closing during a tail-relative final descent.

Closing at `8`, `10`, `12`, or `15 mm` still produced no successes. The first three variants acquired in 6, 2, and
6 environments out of 16; closing at `15 mm` acquired none. The `8 mm` variant was geometrically informative: its
lost-grasp right acquisition mean was approximately `(+11.59,+0.55,-2.29) mm`. Its local-Z coordinate approached
the native successful `-4.53 mm` value, but local-X remained on the wrong side of the native `-5.45 mm` mean.
Threshold and waypoint scans were stopped at this point. A short contact-phase residual was instead expressed in
the current right-hand frame so the correction follows wrist orientation; positive local-X moves the TCP toward the
side required to reduce the measured tail-to-TCP local-X error.

The hand-frame residual had the correct qualitative effect but was too small for success. The no-residual control
acquired 6/16 times and reached a maximum right tail distance of `62.34 mm`. Positive local-X residuals with nominal
cumulative TCP commands of about `10`, `15`, and `18.75 mm` acquired 7, 10, and 8 times and raised the maximum right
tail distance to `66.76`, `62.34`, and `73.58 mm`, respectively. Every acquisition was eventually lost and no exact
success occurred. The strongest candidate moved the previously stalled right tail by more than `11 mm`, so a final
dose scan retained this one direction and increased the cumulative command to `30--45 mm` to test whether the
success distance could be crossed before release.

Increasing the cumulative local-X command did not cross the success boundary. Nominal cumulative commands of
`30`, `30`, `30`, and `45 mm` produced 5, 5, 3, and 2 acquisitions out of 16, with maximum right tail distances of
`62.34`, `69.30`, `72.62`, and `69.10 mm`. No exact success occurred, and stronger or longer corrections reduced
acquisition robustness. Local-X is a useful state-conditioned feature because it moves the otherwise stalled right
tail, but a fixed residual cannot replace contact-aware regrasp. Geometry and residual dose scans were stopped. The
forward training split keeps the 32/32-acquisition wide-aperture approach as an imitation prefix and treats the
post-acquisition socket correction as a contact-anchor DAgger/RL problem, with native level-91 successes retained to
protect the learned pull behavior.

### Behavior cloning, DAgger, and the gripper-reference contract

The imitation stage reused 10,981 transitions from 58 successful native level-91 trajectories and added a scripted
full-reset approach dataset. The first full-reset dataset had been recorded while the command-line tool overrode the
per-finger opening from the policy's `10 mm` proprioceptive reference to `15 mm`. Its finger observations were
therefore shifted by `-0.125` in both finger dimensions: the observation scale is 25 and the physical reference
difference is `5 mm`. Training those samples without correcting their coordinate system produced a policy that
looked accurate offline but failed when evaluated with the production `10 mm` observation baseline.

The reset/action contract was split explicitly rather than changing the old policy's reference globally. Levels 0
through 91 retain a `10 mm` reset aperture, levels 92 through 96 reset at `15 mm` for approach clearance, the binary
open action targets `15 mm`, and both robot initial states keep the learned `10 mm` proprioceptive reference. Old
approach datasets are canonicalized by adding `+0.125` to observation dimensions 28 and 29. A second DAgger pass was
then collected under this production coordinate system at level 96, using the `15 mm` physical reset aperture and a
per-arm close trigger at `10 mm` tail distance. It supplied 25 acquisition prefixes in
`model20_prodref10_bc_dagger2_close10mm_seed1473.pt`.

The accepted BC checkpoint is
`stabilization_inputs/model20_fullreset_bc_dagger2_prodref10_retain20_seed1474.pt`. It mixes the two legacy approach
datasets with offsets `[0.125, 0.125]`, the new DAgger dataset with offset `0.0`, and the successful level-91
retention dataset with weight 20. Offline arm-action RMSE was `0.06915` on approach data and `0.00480` on retention
data; close accuracy was `98.0%` and retention gripper agreement was `100%`. Under fixed production evaluation it
reached bilateral acquisition in 32/32 deterministic level-96 episodes and 64/64 stochastic level-91 episodes, with
34/64 exact level-91 successes. Level 96 still had zero success because the acquired right-hand socket was near
hand-frame `(x,z)=(+7.06,+7.05) mm`, on the opposite side of the cable from the successful native level-91 socket
near `(-5.43,-4.55) mm`.

### PPO normalizer drift and consolidation gates

A four-update PPO smoke using the ordinary normalized actor immediately lost the new approach behavior. Every saved
checkpoint produced zero level-96 acquisitions; level-91 success dropped from 34/64 at the BC input to 4/64 after
one update and zero thereafter. Each MLP parameter had moved only about `1e-5`, but the actor observation-normalizer
mean and standard deviation moved by as much as `0.0394` and `0.0528`. Causal hybrid checkpoints isolated the
mechanism: PPO weights combined with the original BC statistics retained 32/32 level-96 acquisition and 29/64
level-91 success, whereas BC weights combined with the new PPO statistics produced zero level-96 acquisitions and
only 3/64 level-91 successes. Online normalizer drift, rather than the nominal PPO weight step, caused the
catastrophic forgetting.

Repeating one update with `FixedObservationStatisticsMLPModel` kept every actor normalization tensor bit-identical.
The candidate retained 32/32 level-96 acquisition and reached 35/64 level-91 successes. A stricter alternative that
trained only the 12 arm rows of the output layer also preserved 32/32 acquisition, but level-91 success fell to
25/64; concentrating the Adam step in the last layer was not a better consolidation rule. One accidental comparison
omitted `env.coupling_mode=admm`, resolved to the proxy coupler, and terminated predominantly as missed grasp at
step 8. It was rejected before interpretation; comparing the saved `params/env.yaml` files exposed the contract
mismatch.

A final four-update ADMM smoke used the fixed actor statistics, a 240-step horizon, four GPUs with 64 environments
per rank, learning rate `1e-5`, one full-batch epoch, fixed arm standard deviation `0.1`, and a reset mixture of
approximately 50% level 96, 47.5% level 91, and 2.5% level 95. All four checkpoints retained 32/32 deterministic
level-96 acquisition. Their stochastic level-91 success counts were 31, 31, 28, and 29 out of 64. `model_0.pt` was
selected because it also produced the strongest deterministic full-reset progress: maximum left/right tail distance
of `93.59/75.35 mm`, compared with a right-tail maximum near `62.34 mm` for the BC input. It still had zero exact
level-96 success, and later updates did not improve that result. The next intervention therefore starts from
`2026-09-08_05-16-16_bc_dagger2_fixedstats_admm_contact_anchor_l96_l91_h240_lr1e5_4gpu64_4_seed1510/model_0.pt`,
keeps the fixed-normalizer gate, and targets post-acquisition right-socket correction rather than extending this PPO
run blindly.

### Post-acquisition exploration and regrasp diagnosis

The selected fixed-statistics checkpoint was first evaluated at level 96 with action standard deviations of `0.05`,
`0.075`, `0.10`, and `0.15`, using 64 environments per setting. None of the 256 stochastic episodes succeeded.
Acquisition degraded from 63/64 at standard deviation `0.05` to 45/64 at `0.15`. The widest exploration produced a
large left-tail displacement of `176.90 mm` and tail separation of `201.25 mm`, but the right tail remained near its
`62.34 mm` plateau. Isotropic action noise therefore disrupts the reliable approach without exploring the specific
right-hand contact mode needed for success.

A scripted diagnostic then released only the right gripper after bilateral acquisition, moved the right TCP in its
local hand frame, closed again, and returned control to the policy. With a `10 mm` clearance waypoint, commanded
`(+8,+8)`, `(+12,+12)`, `(+16,+12)`, and `(+12,+16) mm` local-X/Z offsets all produced 0/16 successes. The realized
TCP changes were only about `0.4--3.2 mm` in the commanded axes, and the recaptured right socket remained near
local `x=+7 mm, z=+7--9 mm`. Removing the clearance and testing all four signs of a `12 mm` X/Z command exposed
strong contact coupling: `(+X,-Z)` produced the greatest tail progress (`95.07/77.78 mm` left/right), while the
other signs improved only one socket coordinate at a time. Every variant still had zero successes.

A final positive-X-dominant scan tested `(+24,+6)`, `(+24,+12)`, `(+36,+12)`, and `(+48,+12) mm` commands with a
60-step direct move. All variants acquired in 16/16 environments and subsequently lost the grasp in 16/16. The
right-tail maximum was exactly `62.34 mm` in every setting. Increasing the X command from 24 to 48 mm made the
realized local-X TCP displacement more negative (about `-3.1` to `-4.6 mm`) and moved the completed socket farther
to the wrong side (`x=+9.6` to `+11.8 mm`). This rejects insufficient command magnitude as the explanation: the
post-contact Cartesian correction is not a controllable expert action under the current IK and two-way contact
constraints. Manual regrasp trajectory search was stopped. Subsequent optimization uses low-learning-rate PPO on
the selected checkpoint, fixed observation statistics, level-96-focused resets, and level-91 replay protection.

### Low-learning-rate consolidation and termination timing

The first low-learning-rate run was mislabeled as 75% level 96: setting `terminal_level_fraction=0.25` actually
sampled 25% level 96 and 75% replay, because the terminal fraction is the final-level share rather than the replay
share. Its four fixed-statistics updates at learning rate `2.5e-6` retained 32/32 deterministic level-96
acquisitions. `model_3.pt` was the best balanced candidate: its maximum level-96 left/right tail distances were
`85.09/80.69 mm`, and it retained 30/64 stochastic level-91 successes. This checkpoint became the input to the
subsequent controlled comparisons.

With the intended 75% terminal-level share, online success increased to about 9.5%, but all four independent
level-96 gates remained 0/32. The right tail stayed between `62.34` and `67.48 mm`. Relaxing only the
insufficient-separation deadline to 10,000 steps allowed much larger total separation and left-tail displacement,
including more than `200 mm` separation in some checkpoints, but still produced no level-96 success and no right
tail distance above `78.09 mm`. This established that the original 64-step separation deadline was premature, but
also that removing it could not fix the asymmetric right-hand contact mode.

### Contact-socket acquisition contract

The acquisition definition used only tail-to-TCP Euclidean distance and finger aperture. It therefore treated the
full-reset right-hand socket near `(x,z)=(+7,+8) mm` as equivalent to the load-bearing level-91 socket near
`(-5.4,-4.6) mm`. Once this false acquisition latched, dense reward switched immediately from approach to pull and
the policy received no state signal explaining why the right tail stayed near `62.34 mm`.

The task now expresses acquisition in the controlling hand frames. The targets measured from successful native
level-91 rollouts are `(-2.35,-0.16,+6.98) mm` on the left and `(-5.42,-1.44,-4.62) mm` on the right. A `12 mm`
maximum socket error gates acquisition, while a smooth `20 mm` score supplies dense contact-region progress. The
strict socket is used only to latch acquisition; after latching, grasp retention intentionally returns to the wider
Euclidean condition so normal cable motion under load is not rejected. The success condition itself is unchanged.

A one-update real-manager smoke confirmed that the new contract remained reachable: the level-91 replay produced
nonzero acquisition and success, while deterministic level 96 retained 32/32 Euclidean closure but its wrong socket
was no longer a valid strict acquisition. The checkpoint preserved 29/64 stochastic level-91 successes. Four
low-learning-rate updates with a 32-step acquisition deadline produced zero independent level-96 successes; every
checkpoint kept the wrong right socket and the `62.34 mm` right-tail plateau. Online missed-grasp reached about 66%,
showing that the deadline began when both tails entered the reach region and expired around the time the slow binary
gripper finished closing.

The acquisition deadline was therefore extended to 96 control steps. This removed missed-grasp almost entirely
from the first rollout without introducing unsafe states, but later rollouts still converged to 63--70% missed
grasp and all four deterministic level-96 gates remained 0/32. Their right socket stayed near
`x=+5.6--+6.8 mm, z=+8.2--+9.0 mm`. The longer horizon fixes premature truncation but does not itself solve contact
selection.

Inspection of the dense expression exposed a second credit-assignment error: socket score was multiplied by finger
aperture, so it was exactly zero while the gripper was open. By the time the policy received directional socket
credit, it had already closed into a two-way contact constraint where scripted lateral correction was shown to be
uncontrollable. The approach potential now averages radial reach and contact-socket quality, providing a smooth
contact-side signal before closure without defining an EEF trajectory, waypoint timing, or action replay. A focused
unit test verifies that moving an open gripper toward the socket produces positive dense progress.

Four updates at learning rate `1e-5` with this pre-contact signal remained finite and preserved 32/32 Euclidean
level-96 acquisition, but still yielded zero exact successes. The right socket remained on the wrong side and the
right-tail maximum again stayed near `62.34 mm`. A final one-update ablation increased the internal approach weight
from `0.1` to `1.0`; its right socket remained near `(+6.7,+8.0) mm` with 0/32 success. The larger weight was
rejected and was not made the default. This rules out insufficient scalar reward magnitude at this horizon. The
next shortest intervention should expose the hand-frame socket error directly to the actor or add pre-contact
socket-correct expert labels, rather than adding more PPO iterations to the rejected checkpoints.

### Direct socket observation and expert-feasibility checks

The actor observation now appends the signed left and right hand-frame socket errors. This expands the actor input
from 62 to 68 values and the asymmetric critic input from 63 to 69 values. The migration utility
`scripts/demos/shoelace_expand_socket_observation.py` inserts zero-initialized input columns, initializes the new
normalizer entries with zero mean and unit variance, clears optimizer state, and preserves every old output when the
new observations are zero. The converted fixed-statistics checkpoint produced exactly zero actor and critic output
delta in this regression check.

A zero-update simulator gate then separated migration correctness from learning. The converted checkpoint retained
32/32 deterministic level-96 Euclidean closures and 26/64 stochastic level-91 successes. The full-reset right socket
remained near the known incorrect side, while successful level-91 episodes retained the target socket near
`(-5.5,-1.4,-4.5) mm`. The new coordinates are therefore aligned with the runtime hand frames and do not invalidate
the retained contact policy.

One 61,440-transition update with fixed observation statistics and learning rate `1e-5` retained 35/64 stochastic
level-91 successes, but the deterministic level-96 gate still produced 0/32 successes. Three more updates became
progressively less stable: online missed-grasp rose to 57.1%, the deterministic acquisition count fell to 25/32,
and the right-tail maximum returned to approximately `62.4 mm`. All four candidates were rejected. Direct socket
observation fixes state aliasing, but zero-initialized columns do not create a useful socket-correction behavior from
sparse terminal PPO data quickly enough.

Pure scripted Cartesian socket targeting was also rejected as an expert. A direct tail-relative move from level 96
diverged before acquisition. Routing through the collision-free level-95, level-94, and level-93 reset poses reduced
the right TCP distance to about `15.7 mm`, but still did not acquire the cable. Under differential IK and ADMM
two-way contact, a geometrically specified endpoint is not by itself a dynamically valid demonstration.

Finally, the converted policy was evaluated with a distance-triggered `10 mm` close command and constant right-hand
translation residuals during the first 260 policy steps. Positive local Z moved one socket coordinate in the desired
direction, whereas positive local X worsened the measured error. The paired negative-X/positive-Z scan tested
normalized-action residuals `(-0.02,0,+0.02)`, `(-0.04,0,+0.02)`, `(-0.08,0,+0.02)`, and
`(-0.04,0,+0.04)` with an identical level-96 seed. Every variant produced zero bilateral acquisitions and zero
successes. The `(-0.08,0,+0.02)` variant ended
about `84 mm` from the right tail; even the weakest variant never crossed the close threshold. Constant residuals
therefore destroy the reliable approach before they can correct the socket. Further fixed-offset scanning was
stopped. The next expert test must close the feedback loop on the live socket error and apply only a bounded
pre-contact correction, with closure conditioned on both radial reach and socket quality.

The closed-loop expert test preserved the retained checkpoint's orientation and pull outputs, added a bounded
right-hand translation correction only inside a `30 mm` pre-contact region, and required both radial reach and socket
quality before closing. Isotropic gains of `5`, `10`, `20`, and `40 1/m` all produced zero acquisitions. Gain 5
ended at approximately `12.3 mm` radial distance, but its right socket remained near
`(+11.0,-4.4,+3.4) mm`; larger gains moved the hand away from the cable.

Because the fixed residual scan had shown a reversed local-X contact response, a second feedback batch inverted only
that axis and tested gains `2.5`, `5`, `10`, and `20 1/m`. It used a `15 mm` close-distance window while retaining
the strict `12 mm` socket-error gate. The lowest gain let the left gripper close and ended at right radial distance
`10.6 mm`, but the right socket was still near `(+10.4,-1.2,+1.8) mm` and the right gripper correctly remained
open. Higher gains again diverged. A proportional Cartesian correction cannot change which side of the cable the
level-96 approach enters. These rollouts were not saved as demonstrations. The shortest remaining learning target is
the first reset level where this contact topology changes, rather than additional residual tuning at level 96.
