...
## Current controlled shoe-collider ablation

The current two-cable script was measured twice over 120 synchronized simulation steps. Only the shoe collision
mesh changed.

| Shoe collider | Triangles/environment | Mean FPS | Delta | Environment steps/s |
|---|---:|---:|---:|---:|
| Original full shell | 28,106 | 0.8106 | — | 3,320 |
| Simplified closed shell | 8,000 | 0.8520 | +5.1% | 3,490 |

The simplified collider removes 71.5% of shoe triangles and reduces mean frame time by 4.9%. The rendered shoe
is unchanged because it uses a separate visual asset.
...
## Earlier observed configuration delta
...
- The original collider measured `0.810628` and `0.810501 FPS`; the simplified collider measured `0.854137` and `0.849787 FPS`.
- The command was `uv run python scripts/demos/newton_shoelace.py --num_envs 4096 --visualizer none --max_steps 120`; `TRIANGLE_PAIRS_PER_ENV=8192` and `CONTACT_BUFFER=128` were fixed script configuration.
- Synchronized timing excluded USD loading, model construction, solver initialization, and CUDA graph capture.
- All current controlled runs used an NVIDIA RTX 5880 Ada Generation GPU.
- The earlier `0.964163 FPS` result remains in the development table, but it is not directly comparable because the script configuration changed afterward.
...
- Both earlier controlled collision-mesh runs were single measurements; no repeat variance is available.
