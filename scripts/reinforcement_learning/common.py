...
def prelaunch_kit_before_cfg(args_cli: argparse.Namespace, argv: list[str]):
    ...
    Building some task configs (notably the LIBERO multitask configs) imports torch and
    assembles many sub-tasks, which loads ``libgomp``/``libtorch`` into the base linker
    namespace. If that heavy work runs before carb creates its private ``dlmopen``
    namespace, Kit aborts with ``_dl_find_dso_for_object: Assertion 'ns == l->l_ns'``.
    Launching Kit first establishes the namespace so the config build is safe;
    :func:`~isaaclab.app.launch_simulation` then becomes a no-op relaunch via its
    ``has_kit()`` guard.

    The decision uses only the CLI (the config is not built yet). Kit-based backends
    (PhysX/ovphysx) and the Kit visualizer need Isaac Sim, while a Newton backend without
    a Kit visualizer is kitless; only genuinely kitless runs skip the early launch.
